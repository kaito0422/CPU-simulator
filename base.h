...
enum InstOp { ADD, SUB, MUL, DIV, LDR, STR, JMP, MOV };
enum class InstDataType { INT, FLOAT };
enum InstReg { R0, R1, R2, R3, R4, R5, R6, R7, R8, R9, RX };
// register RX is for nothing

typedef struct instInfo {
	InstOp op;								// opearation type
	InstDataType dt;						// data operated type
	int reg_src1, reg_src2, reg_dst;	// used registers
